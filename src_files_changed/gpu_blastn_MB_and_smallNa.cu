...
#endif
	cout << "Was here, should print if GPU is run, 8_1mod4" << endl;
	...
	Int4* scan_range)
{
	...
	slogfile.KernelEnd();
	slogfile.addTotalTime("scan_kernel_time", slogfile.KernelElaplsedTime(), false);
    cout << "[1] Was here. If LOG_TIME==1 then I should be printed out -- scan_kernel_time" << endl;
#endif			   
    cout << "[2] Was here. I should be printed out -- IF GPU scan is run" << endl;
	checkCudaErrors(cudaMemcpy(&total_hits, &p_scanMultiDBAuxWrap->counter[0], sizeof(Uint4), cudaMemcpyDeviceToHost));
	//cout << total_hits <<"\n";
	//checkCudaErrors(cudaMemcpy(&total_hits, &p_scanMultiDBAuxWrap->counter[1], sizeof(Uint4), cudaMemcpyDeviceToHost));
	//cout << total_hits <<"\n";	
	...
	Int4* scan_range)
{
	...
	slogfile.KernelEnd();
	slogfile.addTotalTime("scan_kernel_time", slogfile.KernelElaplsedTime(),false);
	cout << "Was here, gpu_blastn_scan_11_2mod4_v3, Opt_v3_1" << endl;
	...
	Int4* scan_range)
{
	...
	// Copy data from d_lookupArray to c_lookupArray before launching the kernel
cudaMemcpyToSymbol(c_lookupArray,p_MBHashWrap->lookupArray, sizeof(PV_ARRAY_TYPE) * CONSTANT_MEM_SIZE);
	slogfile.KernelStart();
